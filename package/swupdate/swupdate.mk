...
SWUPDATE_SOURCE = swupdate-$(SWUPDATE_VERSION).tar.xz
...
SWUPDATE_CONF_OPTS += -L$(TARGET_DIR)/usr/lib

SWUPDATE_BUILD_CONFIG = $(@D)/.config

SWUPDATE_CFLAGS += $(TARGET_CFLAGS)
SWUPDATE_CFLAGS += -I$(STAGING_DIR)/usr/include
SWUPDATE_CFLAGS += -I$(STAGING_DIR)/include

#SWUPDATE_KCONFIG_FILE = $(call qstrip,$(BR2_PACKAGE_SWUPDATE_CONFIG))
#SWUPDATE_KCONFIG_FILE = package/swupdate/swupdate.config
...
define SWUPDATE_BUILD_CMDS
	rm -f $(@D)/.configured*
	grep 'SWUPDATE_CONFIG' $(BASE_DIR)/.config | sed -e "s,\\(# \)\\?SWUPDATE_CONFIG_\\(.*\\),\\1CONFIG_\\2,g" > $(SWUPDATE_BUILD_CONFIG)
	yes '' | $(MAKE) -C $(@D) oldconfig
	#$(TARGET_MAKE_ENV) $(SWUPDATE_MAKE_ENV) $(MAKE) -C $(@D) LDFLAGS="$(SWUPDATE_CONF_OPTS)"
	$(TARGET_MAKE_ENV) $(SWUPDATE_MAKE_ENV) $(MAKE) -C $(@D) \
		MAKE="$(MAKE)" \
		ARCH="$(TARGET_ARCH)" \
		CC="$(TARGET_CC)" \
		CROSS_COMPILE="$(TARGET_CROSS)" \
		KBUILD_CFLAGS="$(SWUPDATE_CFLAGS)" \
		LD="$(TARGET_CC)" \
		LDFLAGS="$(SWUPDATE_CONF_OPTS)"
endef

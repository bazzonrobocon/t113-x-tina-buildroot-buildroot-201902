...
build_external()
{
    ...
    echo $LICHEE_TOP_DIR
    echo $LICHEE_FLASH
    if [ -d $LICHEE_TOP_DIR/buildroot/package/auto ];  then
               if [ "x${LICHEE_FLASH}" = "xnor" ]; then
                       echo "skip $LICHEE_PLATFORM_DIR/buildroot/package/auto/build.sh for NOR flash"
               else
                       echo "build auto ..."
                       $LICHEE_TOP_DIR/buildroot/package/auto/build.sh
               fi
       fi
}
